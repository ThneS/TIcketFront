...
import { Show, PaginatedShows } from '../../types/show';
...
describe('Shows API', () => {
  ...
  describe('fetchShows', () => {
    it('should fetch shows with no parameters', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/show');
    ...
    it('should fetch shows with pagination parameters', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/show?page=2&pageSize=10');
      expect(result).toEqual(mockShows);
    });
    ...
    it('should handle paginated response format', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/show?page=1&pageSize=2');
    ...
    it('should handle API errors', async () => {
      ...
      await expect(fetchShows()).rejects.toThrow('API 500: Internal Server Error');
    });
    ...
    it('should correctly serialize query parameters', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/show?page=5');

      // Test with only pageSize
      await fetchShows({ pageSize: 25 });
      expect(mockGet).toHaveBeenCalledWith('/show?pageSize=25');

      // Test with both
      await fetchShows({ page: 3, pageSize: 15 });
      expect(mockGet).toHaveBeenCalledWith('/show?page=3&pageSize=15');
    });
  });
  ...
  describe('fetchShow', () => {
    ...
    it('should handle 404 errors for non-existent shows', async () => {
      ...
      await expect(fetchShow('non-existent')).rejects.toThrow('API 404: Show not found');
    });
